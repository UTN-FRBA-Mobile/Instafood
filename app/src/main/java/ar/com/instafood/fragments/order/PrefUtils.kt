...
class PrefUtil {
    companion object {
        ...
        fun getTimerState(context: Context): OrderFragment.TimerState {
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return OrderFragment.TimerState.values()[ordinal]
        }

        fun setTimerState(state: OrderFragment.TimerState, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
